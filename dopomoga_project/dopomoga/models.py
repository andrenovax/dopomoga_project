...
class Review(models.Model):
    ...
    pictures = models.ImageField(upload_to='project_descr_images',null=True,blank=True)
    #status
...
class Cause(models.Model):
    ...
    name = models.CharField(max_length=128, unique=True)
    descr = models.CharField(max_length=2048, blank=True)
    picture = models.ImageField(upload_to='cause_images', blank=True)
    def __unicode__(self):
        return self.name
...
class UserProfile(models.Model): #the same attrs as UserHelper, because any user can be helper, but including users inneed in the same table isirrational
    # relations
